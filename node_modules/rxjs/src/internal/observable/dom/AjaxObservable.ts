...
export class AjaxObservable<T> extends Observable<T> {
  ...
  */
  static create: AjaxCreationMethod = (() => {
    ...
    return create as AjaxCreationMethod;
  })();
...
export class AjaxSubscriber<T> extends Subscriber<Event> {
  ...
  private setupEvents(xhr: XMLHttpRequest, request: AjaxRequest) {
    ...
    function xhrTimeout(this: XMLHttpRequest, e: ProgressEvent) {
      const {subscriber, progressSubscriber, request } = (xhrTimeout as any);
    ...
    (xhrTimeout as any).request = request;
    (xhrTimeout as any).subscriber = this;
    (xhrTimeout as any).progressSubscriber = progressSubscriber;
    if (xhr.upload && 'withCredentials' in xhr) {
      if (progressSubscriber) {
        let xhrProgress: (e: ProgressEvent) => void;
        xhrProgress = function(e: ProgressEvent) {
          const { progressSubscriber } = (xhrProgress as any);
        ...
        (xhrProgress as any).progressSubscriber = progressSubscriber;
      }
      let xhrError: (e: any) => void;
      xhrError = function(this: XMLHttpRequest, e: ErrorEvent) {
        const { progressSubscriber, subscriber, request } = (xhrError as any);
        if (progressSubscriber) {
          progressSubscriber.error(e);
        }
      ...
      (xhrError as any).request = request;
      (xhrError as any).subscriber = this;
      (xhrError as any).progressSubscriber = progressSubscriber;
    }
    ...
    (xhrReadyStateChange as any).subscriber = this;
    (xhrReadyStateChange as any).progressSubscriber = progressSubscriber;
    (xhrReadyStateChange as any).request = request;

    function xhrLoad(this: XMLHttpRequest, e: Event) {
      const { subscriber, progressSubscriber, request } = (xhrLoad as any);
      if (this.readyState === 4) {
        // normalize IE9 bug (http://bugs.jquery.com/ticket/1450)
        let status: number = this.status === 1223 ? 204 : this.status;
        const response: any = (this.responseType === 'text' ?  (
    ...
    xhr.onload = xhrLoad;
    (xhrLoad as any).subscriber = this;
    (xhrLoad as any).progressSubscriber = progressSubscriber;
    (xhrLoad as any).request = request;
  }

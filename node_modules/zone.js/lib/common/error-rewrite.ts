...
Zone.__load_patch('Error', (global: any, Zone: ZoneType, api: _ZonePrivate) => {
  ...
  function buildZoneAwareStackFrames(
    ...
    const frames: string[] = originalStack.split('\n');
    let i = 0;
  ...
  function ZoneAwareError(): Error {
    ...
    const error: Error = NativeError.apply(this, arguments);
    ...
    if ((ZoneAwareError as any)[stackRewrite] && originalStack) {
      const zoneFrame = api.currentZoneFrame();
  ...
  const detectZone: Zone = Zone.current.fork({
    name: 'detect',
    onHandleError(
        parentZD: ZoneDelegate, current: Zone, target: Zone, error: any): boolean {
      if (error.originalStack && Error === ZoneAwareError) {
        const frames = error.originalStack.split(/\n/);
        ...
        while (frames.length) {
          ...
          if (/:\d+:\d+/.test(frame) || frame === 'ZoneAwareError') {
            ...
            const fnName: string = frame.split('(')[0].split('@')[0];

...
Zone.__load_patch('ZoneAwarePromise', (global: any, Zone: ZoneType, api: _ZonePrivate) => {
    ...
    if (api.showUncaughtError()) {
      ...
      if (rejection) {
        console.error(
            ...
            '; Zone:', (e.zone as Zone).name, '; Task:', e.task && (e.task as Task).source,
  ...
  function resolvePromise(
    ...
    if ((promise as any)[symbolState] === UNRESOLVED) {
      ...
      if (state !== REJECTED && value instanceof ZoneAwarePromise &&
        ...
        clearRejectedNoCatch(value as Promise<any>);
  ...
  function clearRejectedNoCatch(promise: ZoneAwarePromise<any>): void {
    if ((promise as any)[symbolState] === REJECTED_NO_CATCH) {
      ...
      try {
        ...
        if (handler && typeof handler === 'function') {
          handler.call(this, {rejection: (promise as any)[symbolValue], promise});
        }
      } catch (err) {
      }
  ...
  class ZoneAwarePromise<R> implements Promise<R> {
    ...
    static resolve<R>(value: R): Promise<R> {
      return resolvePromise(new this(null as any) as ZoneAwarePromise<R>, RESOLVED, value);
    }

    static reject<U>(error: U): Promise<U> {
      return resolvePromise(new this(null as any) as ZoneAwarePromise<U>, REJECTED, error);
    }
    ...
    static all<R>(values: any): Promise<R> {
      let resolve: (v: any) => void;
      let reject: (v: any) => void;
      const promise = new this<R>((res, rej) => {
        resolve = res;
        reject = rej;
      });
    ...
    then<TResult1 = R, TResult2 = never>(
      ...
      if ((this as any)[symbolState] == UNRESOLVED) {
        ((this as any)[symbolValue] as any[]).push(zone, chainPromise, onFulfilled, onRejected);
      } else {
      ...
      const zone = Zone.current;
      if ((this as any)[symbolState] == UNRESOLVED) {
        ((this as any)[symbolValue] as any[]).push(zone, chainPromise, onFinally, onFinally);
      } else {
  ...
  function zoneify(fn: Function) {
    return function() {
      const resultPromise = fn.apply(this, arguments);
      if (resultPromise instanceof ZoneAwarePromise) {
        return resultPromise;
      }
      const ctor = resultPromise.constructor;

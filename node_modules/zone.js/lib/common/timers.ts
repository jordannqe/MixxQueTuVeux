...
export function patchTimer(window: any, setName: string, cancelName: string, nameSuffix: string) {
  ...
  function scheduleTask(task: Task) {
    const data = task.data as TimerOptions;
    function timer() {
      try {
  ...
  function clearTask(task: Task) {
    return clearNative!((task.data as TimerOptions).handleId);
  }
  ...
  setNative =
      patchMethod(window, setName, (delegate: Function) => function(self: any, args: any[]) {
        if (typeof args[0] === 'function') {
          ...
          const handle: any = (task.data as TimerOptions).handleId;
          ...
          if (handle && handle.ref && handle.unref && typeof handle.ref === 'function' &&
            ...
            (task as any).ref = (handle as any).ref.bind(handle);
            (task as any).unref = (handle as any).unref.bind(handle);
          }

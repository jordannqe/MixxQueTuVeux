...
class Scheduler {
  ...
  scheduleFunction(
    ...
    const currentId: number = id < 0 ? this.nextId++ : id;
    const endTime = this._currentTime + delay;

    // Insert so that scheduler queue remains sorted by end time.
    const newEntry: ScheduledFunction = {
      endTime,
      id: currentId,
      func: cb,
      args,
    ...
    for (; i < this._schedulerQueue.length; i++) {
      const currentEntry = this._schedulerQueue[i];
  ...
  tick(millis: number = 0, doTick?: (elapsed: number) => void): void {
    const finalTime = this._currentTime + millis;
    ...
    while (this._schedulerQueue.length > 0) {
      const current = this._schedulerQueue[0];
...
class FakeAsyncTestZoneSpec implements ZoneSpec {
  ...
  private _setTimeout(fn: Function, delay: number, args: any[], isTimer = true): number {
    const removeTimerFn = this._dequeueTimer(this._scheduler.nextId);
    // Queue the callback and dequeue the timer on success and error.
    const cb = this._fnAndFlush(fn, {onSuccess: removeTimerFn, onError: removeTimerFn});
    const id = this._scheduler.scheduleFunction(cb, delay, args, false, !isTimer);
    if (isTimer) {
  ...
  private _setInterval(fn: Function, interval: number, args: any[]): number {
    const id = this._scheduler.nextId;
    const completers = {onSuccess: null as any, onError: this._dequeuePeriodicTimer(id)};
    const cb = this._fnAndFlush(fn, completers);
  ...
  private _resetLastErrorAndThrow(): void {
    const error = this._lastError || this._uncaughtPromiseErrors[0];
  ...
  onScheduleTask(delegate: ZoneDelegate, current: Zone, target: Zone, task: Task): Task {
    switch (task.type) {
      case 'microTask':
        ...
        if (args) {
          const callbackIndex = (task.data as any).cbIdx;
        ...
        switch (task.source) {
          ...
          default:
            ...
            if (macroTaskOption) {
              ...
              const callbackArgs = macroTaskOption.callbackArgs ? macroTaskOption.callbackArgs : args;
  ...
  onCancelTask(delegate: ZoneDelegate, current: Zone, target: Zone, task: Task): any {
    switch (task.source) {
      ...
      case 'webkitRequestAnimationFrame':
      case 'mozRequestAnimationFrame':
        return this._clearTimeout(task.data!.handleId as number);
      case 'setInterval':
        return this._clearInterval(task.data!.handleId as number);
      default:
        // user can define which macroTask they want to support by passing
        // macroTaskOptions
        const macroTaskOption = this.findMacroTaskOption(task);
        if (macroTaskOption) {
          const handleId: number = task.data!.handleId as number;
